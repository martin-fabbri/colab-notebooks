...
def plot_contours(ax, clf, xx, yy, proba=False, transformation=None, labels=None, **params):
    ...
    if proba == "raw":
        ...
        Z = Z.reshape(xx.shape)
        out = ax.imshow(Z, extent=(np.min(xx), np.max(xx), np.min(yy), np.max(yy)), origin='lower', vmin=0, vmax=1,
                        aspect="auto", **params)
...
def plot_classifier(X, y, clf, ax=None, ticks=False, proba=False, lims=None, transformation=None, show_data=True,
                    gray_photocopy=False, proba_showtitle=True, **kwargs):  # assumes classifier "clf" is already fit
    try:
        X = X.to_numpy()
    except:
        pass
